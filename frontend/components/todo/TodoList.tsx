...
export default async function TodoList({
  ...
  const sortedByCreatedDate = todos.data?.slice().sort((a, b) => {
    return (
      new Date(
        searchParams.sort?.includes("-") ? b.created_date : a.created_date
      ).getTime() -
      new Date(
        searchParams.sort?.includes("-") ? a.created_date : b.created_date
      ).getTime()
    );
  });
  ...
  const sortedByUpdatedDate = todos.data?.slice().sort((a, b) => {
    return (
      new Date(
        searchParams.sort?.includes("-") ? b.updated_date : a.updated_date
      ).getTime() -
      new Date(
        searchParams.sort?.includes("-") ? a.updated_date : b.updated_date
      ).getTime()
    );
  });
        ...
        <main className="flex flex-col gap-4">
          <div className="flex justify-between gap-2 sm:gap-4 items-center px-2">
            ...
            <div className="flex gap-4">
              <Link
                href={`/todos?sort=${createSortQuery(searchParams.sort)}`}
                className="text-xs font-light hover:underline"
              >
                Created Date
              </Link>
              <Link
                href={`/todos?sort=${updateSortQuery(searchParams.sort)}`}
                className="text-xs font-light hover:underline"
              >
                Updated Date
              </Link>
